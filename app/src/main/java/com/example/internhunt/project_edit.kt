...
class project_edit : AppCompatActivity() {
    ...
    private var project : ProjectsEntry? = null
...
//        enableEdgeToEdge()
        ...
        Log.d("project_edit", "Received position: $position")
        project = intent.getSerializableExtra("project") as? ProjectsEntry
        Log.d("project_edit", "Received project: $project")
        var title = project?.title
        var description = project?.description
        var technologies = project?.technologies?.joinToString(", ")
        ...
        etTechnologies.setText(technologies)
        etLink.setText(link)
        etStartMonth.setText(startDate?.split("/")?.get(0))
        etStartYear.setText(startDate?.split("/")?.get(1))

        if (endDate != "Present"){
            etEndMonth.setText(endDate?.split("/")?.get(0))
            etEndYear.setText(endDate?.split("/")?.get(1))
        }
...
// End Date validation (Optional)
    ...
    private fun updateProject(userId: String, position: Int) {
        ...
        val projectStartDate = etStartMonth.text.toString() + "/" + etStartYear.text.toString()
        ...
        val technologiesList = projectTechnologiesInput.split(",").map { it.trim() }.filter { it.isNotEmpty() }

        val updatedProject = hashMapOf(
        ...
        userDocRef.get().addOnSuccessListener { doc ->
            val projectsList = doc.get("projects") as? MutableList<HashMap<String, Any>> ?: mutableListOf()

            if (position in projectsList.indices) {
                // 2. Update the project at the given position
                projectsList[position] = updatedProject

                // 3. Write the updated array back to Firestore
                userDocRef.update("projects", projectsList)
                ...
                progressBar.visibility = View.GONE
                Toast.makeText(this, "Invalid project position", Toast.LENGTH_SHORT).show()
            }
        }.addOnFailureListener { e ->
            progressBar.visibility = View.GONE
            Toast.makeText(this, "Error fetching projects: ${e.message}", Toast.LENGTH_SHORT).show()
        }
    }

    private fun deleteProject(userId: String, position: Int) {
        val db = FirebaseFirestore.getInstance()
        val userDocRef = db.collection("Users").document(userId)

        // 1. Get the existing projects array
        userDocRef.get().addOnSuccessListener { doc ->
            val projectsList = doc.get("projects") as? MutableList<HashMap<String, Any>> ?: mutableListOf()

            if (position in projectsList.indices) {
                // 2. Remove the project at the given position
                ...
                userDocRef.update("projects", projectsList)
                    ...
                    .addOnFailureListener { e ->
                        hideKeyboard(etProjectTitle)
                        Toast.makeText(this, "Error deleting project: ${e.message}", Toast.LENGTH_SHORT).show()
                    }
            } else {
                hideKeyboard(etProjectTitle)
                Toast.makeText(this, "Invalid project position", Toast.LENGTH_SHORT).show()
